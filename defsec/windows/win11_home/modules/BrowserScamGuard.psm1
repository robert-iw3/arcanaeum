...
.DESCRIPTION
    A large share of the scams that lead to a compromise (ClickFix, fake tech-support alerts,
    fake "your computer is infected" pages) are delivered entirely inside the browser before
    anything ever touches disk:
        ...
        - A malicious or compromised site asks for notification permission behind a disguised
          ...
          push OS-style popups that look like real virus alerts indefinitely, even after the tab
          is closed. This is the actual delivery mechanism behind most "your computer has a
          virus, call this number" popups.
        - The browser's phishing/malware warning is what would normally stop a user from
    ...
    Ported verbatim from the applocker baseline's module of the same name - it was policy/
    registry-only there and needs no AppLocker. Sets the Chromium policy equivalents for Edge
    and Chrome via HKLM, and the matching Firefox enterprise policies.json. Setting a value for
    a browser that isn't installed is harmless - the browser picks it up if/when installed.

    Balance note: this hardens the scam-popup path, not general browsing. It does not block
    sites, extensions, or downloads a normal user relies on.
#>
