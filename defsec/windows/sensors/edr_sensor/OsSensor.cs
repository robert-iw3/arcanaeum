...
public class DeepVisibilitySensor {
    ...
    public static void SuppressProcessRule(string process, string ruleName) {
        if (string.IsNullOrWhiteSpace(process) || string.IsNullOrWhiteSpace(ruleName)) return;
    ...
    // Non-mutating read of the same tally GetRepeatActorScoreDelta maintains. Scoring
    // always runs before containment dispatch for the same alert and self-registers the
    // current alert into the tally, so this returns prior alerts only (Count - 1) --
    // otherwise the current alert would count as its own corroboration.
    private static int GetRepeatActorCount(int pid) {
        long nowTicks = DateTime.UtcNow.Ticks;
        if (_actorRecentAlerts.TryGetValue(pid, out ActorTally cur)) {
            if (nowTicks - cur.FirstTicks > _actorWindowTicks) return 0;
            return cur.Count > 0 ? cur.Count - 1 : 0;
        }
        return 0;
    }
    ...
    // Scans a forensic dump file (produced by PreserveForensics) against every compiled YARA
    // vector. Used as a corroboration gate in DispatchContainment: a Sigma rule match alone can
    // be a false positive (rule bugs, overly broad patterns), so before actually terminating a
    // PID/TID we check whether the captured memory content itself contains a real signature hit.
    public static string ScanDumpWithYara(string dumpPath) {
        if (string.IsNullOrEmpty(dumpPath) || !System.IO.File.Exists(dumpPath)) return "NoDump";
        try {
            lock (_yaraLock) {
                if (_yaraContext == null) return "NoDump";
                var scanner = new libyaraNET.Scanner();
                List<string> matches = new List<string>();
                foreach (var vector in YaraMatrices.Values) {
                    var results = scanner.ScanFile(dumpPath, vector);
                    if (results != null && results.Count > 0) {
    ...
    public static string EvaluatePayloadInMemory(byte[] payload, string processName) {
        // Scan every compiled vector, not just the one DetermineThreatVector guesses from the
        // process name -- an injected payload's real signature (e.g. an infostealer or C2
        // implant dropped into powershell.exe) routinely lives outside the "LotL" vector that
        // heuristic would pick, and a single-vector scan silently missed it.
        try {
            var scanner = new libyaraNET.Scanner();
            List<string> matches = new List<string>();
            foreach (var vector in YaraMatrices.Values) {
                var results = scanner.ScanMemory(payload, vector);
                if (results != null && results.Count > 0) {
                    foreach (var match in results) { matches.Add(match.MatchingRule.Identifier); }
                }
            }
    ...
    private static bool IsForgedReturnAddress(int pid, ulong returnAddr) {
        ...
        try {
            if (read_process_memory_region((uint)pid, readAddr, buffer, 10) != 1) return true;
    ...
    // Native mechanism lives in native_defense.rs; policy stays here.
    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern int quarantine_native_thread(uint tid);

    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern int resume_native_thread(uint tid);

    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern int preserve_forensics(uint pid, [MarshalAs(UnmanagedType.LPUTF8Str)] string dumpPath);

    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern int read_process_memory_region(uint pid, ulong address, [In, Out] byte[] buffer, ulong size);

    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern int virtual_protect_noaccess(uint pid, ulong address, ulong size);

    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern IntPtr correlate_on_disk_artifacts(uint pid);

    // JSON blob: ImagePath/ImageSHA256/ParentPID/ParentImage/LoadedModules/Persistence.
    public static string CorrelateOnDiskArtifacts(int pid) {
        IntPtr resultPtr = correlate_on_disk_artifacts((uint)pid);
    ...
    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern uint get_process_parent_pid(uint pid);

    public static int GetParentPid(int pid) {
        try { return (int)get_process_parent_pid((uint)pid); } catch { return 0; }
    }

    [DllImport("DeepSensor_ML_v2.1.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern IntPtr get_builtin_threat_signatures();

    // JSON string array of built-in command-line signatures (Sigma-rule seed data).
    public static string GetBuiltinThreatSignatures() {
    ...
    public static bool QuarantineNativeThread(int tid, int pid) {
        ...
        // Re-check right before the freeze call to keep the PID-reuse window tight.
        ...
        try {
            return quarantine_native_thread((uint)tid) == 1;
        } catch (Exception ex) {
            EnqueueDiag($"[ACTIVE DEFENSE ERROR] quarantine_native_thread FFI call failed: {ex.Message}");
            return false;
        }
    }
    ...
    public static string NeuterAndDumpPayload(int pid, ulong address, ulong size) {
        ...
        try {
            ...
            if (read_process_memory_region((uint)pid, address, buffer, size) == 1) {
                ...
                if (yaraResult != "NoSignatureMatch") {
                    ...
                    if (JitRuntimeProcesses.ContainsKey(procName)) {
                        ...
                        virtual_protect_noaccess((uint)pid, address, size);
                    }
                }
            }
        } catch (Exception ex) {
            EnqueueDiag($"[ACTIVE DEFENSE ERROR] Memory dump failed: {ex.Message}");
    ...
    public static string PreserveForensics(int pid, string procName) {
        ...
        if (CriticalSystemProcesses.ContainsKey(procName)) return "Bypassed";
        ...
        try {
            return preserve_forensics((uint)pid, dumpPath) == 1 ? dumpPath : "Failed";
        } catch (Exception ex) {
            EnqueueDiag($"[ACTIVE DEFENSE ERROR] preserve_forensics FFI call failed: {ex.Message}");
            return "Failed";
        }
    }
    ...
    public static bool ResumeNativeThread(int tid) {
        try {
            return resume_native_thread((uint)tid) == 1;
        } catch (Exception ex) {
            EnqueueDiag($"[ACTIVE DEFENSE ERROR] resume_native_thread FFI call failed: {ex.Message}");
            return false;
        }
    }

    // Full containment decision + execution, called by the launcher's Invoke-ActiveDefense as a
    // single opaque call. Returns {"Status":"...","Artifact":"..."} JSON.
    private static readonly string[] TrustedImagePathPrefixes = {
        @"C:\Windows\System32", @"C:\Windows\SysWOW64", @"C:\Windows\WinSxS",
        @"C:\Program Files", @"C:\Program Files (x86)",
    };

    // Justified if the image is untrusted (unsigned/unknown signature or untrusted path), or
    // this PID has a prior corroborating alert.
    private static bool IsContainmentJustified(int pid, string imageSignature, string imagePath) {
        bool signedAndTrusted = string.Equals(imageSignature, "Valid", StringComparison.OrdinalIgnoreCase)
            && !string.IsNullOrEmpty(imagePath)
            && Array.Exists(TrustedImagePathPrefixes, p => imagePath.StartsWith(p, StringComparison.OrdinalIgnoreCase));

        if (!signedAndTrusted) return true;
        return GetRepeatActorCount(pid) > 0;
    }

    public static string DispatchContainment(string procName, int pid, int tid, string targetType, string imageSignature, string imagePath) {
        if (CriticalSystemProcesses.ContainsKey(procName)) {
            return "{\"Status\":\"Skipped_Critical\",\"Artifact\":\"None\"}";
        }

        string artifact = "None";
        string dump = PreserveForensics(pid, procName);
        if (dump != "Failed" && dump != "AccessDenied" && dump != "Bypassed") { artifact = dump; }

        // YARA corroboration gate: a Sigma rule match alone can be a false positive (rule bugs,
        // overly broad patterns). If we captured a memory dump, scan it before acting -- a clean
        // scan is treated as evidence against the Sigma alert and holds the action (still fully
        // logged, forensics preserved). No dump captured falls through to the existing
        // trust/corroboration check unchanged -- a process that evades dumping must not get a
        // free pass just because we couldn't inspect it.
        if (artifact != "None") {
            string yaraVerdict = ScanDumpWithYara(artifact);
            if (yaraVerdict == "NoSignatureMatch") {
                EnqueueDiag($"[ACTIVE DEFENSE] PID {pid} ({procName}) held -- Sigma alert fired but YARA found no signature match in the captured memory dump; treating as likely false positive pending further evidence.");
                string safeYaraArtifact = artifact.Replace("\\", "\\\\").Replace("\"", "\\\"");
                return $"{{\"Status\":\"Held_YaraNegative\",\"Artifact\":\"{safeYaraArtifact}\"}}";
            }
        }

        if (!IsContainmentJustified(pid, imageSignature, imagePath)) {
            EnqueueDiag($"[ACTIVE DEFENSE] PID {pid} ({procName}) is signed and running from a trusted path with no prior corroborating alert; holding action pending further evidence.");
            string safeHeldArtifact = artifact.Replace("\\", "\\\\").Replace("\"", "\\\"");
            return $"{{\"Status\":\"Held_PendingCorroboration\",\"Artifact\":\"{safeHeldArtifact}\"}}";
        }

        // Distinct, non-generic statuses for every way containment can end without acting --
        // "Failed" alone reads as a malfunction and gets confused with a deliberate corroboration
        // hold (which already has its own Held_* status above). Each branch below states plainly
        // why nothing happened.
        string status = "Failed_Unknown";
        bool contained = false;
        if (targetType == "Thread" && tid > 0) {
            if (QuarantineNativeThread(tid, pid)) {
                status = $"Thread ({tid}) Quarantined";
                contained = true;
            } else {
                status = "Failed_ThreadQuarantineRejected";
                EnqueueDiag($"[ACTIVE DEFENSE] Thread quarantine rejected/failed for TID {tid} (PID {pid}); falling back to process termination.");
            }
        }

        if (!contained) {
            // Fresh OS-level lookup, not the ETW-populated cache -- closes the PID-reuse window
            // opened by the forensics/quarantine calls above.
            string nowName = null;
            try { nowName = System.Diagnostics.Process.GetProcessById(pid).ProcessName + ".exe"; } catch { }

            if (nowName == null) {
                status = "Failed_ProcessAlreadyExited";
                EnqueueDiag($"[ACTIVE DEFENSE] PID {pid} no longer exists by the time containment was attempted; nothing to terminate.");
            } else if (CriticalSystemProcesses.ContainsKey(nowName)) {
                status = "Aborted_PidReusedByCritical";
                EnqueueDiag($"[ACTIVE DEFENSE] PID {pid} re-resolved to {nowName} between trust-check and kill; termination aborted.");
            } else {
                try {
                    System.Diagnostics.Process.GetProcessById(pid).Kill();
                    status = $"Process ({pid}) Terminated";
                } catch (Exception ex) {
                    status = "Failed_TerminationError";
                    EnqueueDiag($"[ACTIVE DEFENSE ERROR] Process termination failed: {ex.Message}");
                }
            }
        }

        string safeArtifact = artifact.Replace("\\", "\\\\").Replace("\"", "\\\"");
        string safeStatus = status.Replace("\\", "\\\\").Replace("\"", "\\\"");
        return $"{{\"Status\":\"{safeStatus}\",\"Artifact\":\"{safeArtifact}\"}}";
    }
    ...
    // Must match Compile-SigmaRulesToBase64's separator characters exactly. Non-printable
    // control characters, since Sigma pattern values can contain any printable character.
    private const char SepRule      = '\x03'; // rule-to-rule
    private const char SepTopLevel  = '\x1E'; // top-level rule fields
    private const char SepBlocks    = '\x1D'; // blocks within a rule
    private const char SepBlockName = '\x1C'; // block name vs. its fields
    private const char SepFields    = '\x1F'; // fields within a block
    private const char SepValues    = '\x02'; // values within a field
    ...
    public static void UpdateSigmaRules(string b64Rules) {
        try {
            ...
            string[] rules = payload.Split(new[] { SepRule }, StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string r in rules) {
                string[] parts = r.Split(new char[] { SepTopLevel }, 7);
                ...
                foreach (string block in blocks) {
                    ...
                    foreach (string field in fields) {
                        ...
                        selection.Conditions.Add(new SigmaConditionItem {
                            ...
                            Values = fParts[3].Split(SepValues)
                        });
                    }
    ...
    {
        // Same null guard as BuildEnrichedJson -- not currently reachable (all callers pass
        // non-null), but kept consistent so a future caller can't reintroduce that NRE here.
        process = process ?? "";
        ...
        string destinationIp = "", int destPort = 0)
    {
        ...
        // File_Event/Registry_Event/Image_Load/Network_Connection PendingSigmaEvent entries
        // never set ParentImage (only Process_Creation does), so parentProcess arrives null
        // here for those categories -- guard both instead of relying on every caller.
        process = process ?? "";
        parentProcess = parentProcess ?? "";
        string procLower = process.ToLowerInvariant();
        string parentLower = parentProcess.ToLowerInvariant();
    ...
    private static void RunEtwCore() {
        ...
        _session.Source.Kernel.StackWalkStack += delegate (StackWalkStackTraceData data) {
            ...
            } catch (Exception ex) { EnqueueDiag($"[ETW ERROR] StackWalkStack parsing failed: {ex.Message}"); }
        };
        ...
        _session.Source.Kernel.VirtualMemAlloc += delegate (VirtualAllocTraceData data) {
            ...
            } catch (Exception ex) { EnqueueDiag($"[ETW ERROR] VirtualMemAlloc parsing failed: {ex.Message}"); }
        };
