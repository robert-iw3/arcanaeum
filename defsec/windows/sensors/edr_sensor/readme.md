...
> [!IMPORTANT]
>
> **Requires PowerShell 7+ (`pwsh`).** The launcher compiles its C# ETW engine (`OsSensor.cs`) at runtime via `Add-Type`, using modern C# (string interpolation, inline `out` variables) that the built-in Windows PowerShell 5.1 compiler cannot build. Launch with `pwsh` 7.4+; Windows PowerShell 5.1 is not supported.
...
* **Gated Active Defense:** A Sigma/TTP match alone never terminates anything. Every containment decision (`DispatchContainment`) first captures a full memory dump, then requires either a positive YARA hit against that dump or a prior corroborating alert for the same PID within a 5-minute window before touching the process — a single rule match against a signed, trusted-path binary is held (`Held_YaraNegative` / `Held_PendingCorroboration`) and only logged. When containment is justified, it prefers surgical thread quarantine (native `SuspendThread` via `QuarantineNativeThread`) and falls back to full process termination only if quarantine is rejected (critical-process/JIT-runtime exclusions, e.g. chrome.exe/pwsh.exe/node.exe) or unavailable. See [Armed-Mode Containment Pipeline](#armed-mode-containment-pipeline) below.
* **Automated Forensic Attribution:** Every containment decision writes a full memory dump before acting and scans it against all ten YARA vectors (`ScanDumpWithYara` for the general Sigma path, `NeuterAndDumpPayload`/`EvaluatePayloadInMemory` for direct memory-injection detection). The resulting attribution (e.g., "CobaltStrike_Beacon") and the held/acted decision are both embedded in the SIEM audit trail and the append-only forensic ledger.
...
```
Once running: `Q` or `Ctrl+C` gracefully terminates the sensor, `I` re-injects staging, `R` rolls back active-defense state (lifts network blocks, resumes the last quarantined thread), and `T` runs the in-process armed-mode live test (`utils\Test-ArmedModeTidSuspension.ps1`, dot-sourced into the running session so it shares the already-loaded `[DeepVisibilitySensor]` type — `Add-Type` types don't cross process boundaries, so the script can't be run standalone from a separate terminal while armed mode is active).

### Armed-Mode Containment Pipeline
A Sigma or TTP match is never sufficient on its own to terminate a process or suspend a thread. Every match that crosses the tiering threshold reaches `DispatchContainment` (`OsSensor.cs`), which runs the same sequence regardless of whether the trigger was a process-creation, registry, file, image-load, or network-connection event:

1. **Critical-process short-circuit.** If the process name is in the built-in critical-system list (`csrss.exe`, `wininit.exe`, `services.exe`, `dwm.exe`, etc.), containment is skipped outright (`Skipped_Critical`) — no dump, no scan, no action.
2. **Forensics first.** A full memory dump is captured via `PreserveForensics` (native `MiniDumpWriteDump` through the Rust FFI boundary) before any containment decision is made, so evidence exists whether or not the sensor ultimately acts.
3. **YARA corroboration gate.** If a dump was captured, it's scanned against all ten compiled YARA vectors (`ScanDumpWithYara`). A clean scan is treated as evidence *against* the Sigma alert — the action is held (`Held_YaraNegative`), fully logged, nothing terminated. A dump that couldn't be captured skips this gate and falls through to step 4 (a process that evades dumping doesn't get a free pass).
4. **Trust/corroboration gate.** If the image is unsigned or running from outside a trusted path (`System32`, `SysWOW64`, `WinSxS`, `Program Files`), containment is justified immediately. If it's signed and trusted, containment additionally requires a prior alert for the *same PID* within the last 5 minutes (`Held_PendingCorroboration` otherwise) — a single noisy rule match against a legitimate trusted binary is never enough by itself.
5. **Containment, thread-first.** If a valid TID was supplied, thread quarantine is attempted first (`QuarantineNativeThread` / native `SuspendThread`) — this freezes the thread and leaves the rest of the process running. It's rejected outright for critical processes and JIT-runtime hosts (`chrome.exe`, `msedge.exe`, `pwsh.exe`, `powershell.exe`, `node.exe`, `java.exe`, `dotnet.exe`, etc. — suspending an arbitrary thread inside a JIT'd runtime risks hanging or crashing it) and falls back to step 6 if rejected or if no TID was available.
6. **Process termination fallback.** A fresh PID lookup re-checks for critical-process reuse (closing the race window opened by steps 2–5) before calling `Process.Kill()`. Every non-success outcome gets its own explicit status rather than a generic failure — `Failed_ProcessAlreadyExited`, `Failed_ThreadQuarantineRejected`, `Failed_TerminationError`, `Aborted_PidReusedByCritical` — so the ledger never conflates "we deliberately held" with "we tried and it didn't work."
7. **Escalation to network containment.** Only an actual thread quarantine or process termination (never a held/failed outcome) feeds `Test-MitigationCluster`, which requires multiple distinct-signature detections against related PIDs across a minimum time span before escalating to a PID/image-scoped firewall block (`Block-ProcessNetwork`, both directions) — a single mitigated PID does not, by itself, trigger network containment.

Every step of this — action taken, artifact path, image hash/signature, parent lineage, loaded modules, on-disk persistence — is written to the append-only `ArmedMode_ForensicLedger.jsonl`, independent of whether the sensor actually acted.
...
| **`C:\ProgramData\DeepSensor\Data\Forensics`** | Full memory dumps (`.dmp`), one per containment decision. | Written by `PreserveForensics` before any armed-mode decision is made, whether or not the sensor ultimately acts. |
| **`C:\ProgramData\DeepSensor\Data\Quarantine`** | Copied artifacts from the background file-scan YARA worker. | Securely stores a copy of any on-disk file that matches a YARA rule during passive scanning (separate from the in-memory containment dumps above). |
| **`C:\ProgramData\DeepSensor\Data\ArmedMode_ForensicLedger.jsonl`** | Append-only JSONL, one line per containment decision. | Records the action taken (or held/failed reason), dump path, image hash/signature, parent lineage, loaded modules, and on-disk persistence for every armed-mode decision. |
| **`C:\ProgramData\DeepSensor\Data\Reports\Incident_*.json`** | Per-incident JSON snapshot. | Same data as the ledger, one file per `IncidentID`, for ad-hoc inspection. |
| **`C:\ProgramData\DeepSensor\Data\TransmissionQueue.db`** | SQLite WAL durable queue. | Every alert is written here before any network attempt; only deleted after a confirmed successful Parquet upload to the gateway, so a hard crash or kill doesn't lose in-flight telemetry. |
