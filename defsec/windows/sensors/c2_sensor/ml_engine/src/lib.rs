...
// ============================================================================
// FLOW STATISTICS — c2_math 8D vector features
// ============================================================================

/// Pre-computed flow statistics passed from evaluate_flow → dispatch_alert_to_gateway
/// so the alert JSON includes all 8 c2_math dimensions without re-computation.
#[derive(Debug, Clone, Default)]
pub struct FlowStats {
    pub outbound_ratio: f64,    // proxy: asymmetry_ratio / (1 + asymmetry_ratio) or 0.75 default
    pub packet_size_mean: f64,  // mean of flow.packet_sizes
    pub packet_size_std: f64,   // std dev of flow.packet_sizes
    pub interval_mean: f64,     // mean of flow.intervals
    pub cv: f64,                // coefficient of variation of intervals (std/mean)
    pub entropy: f64,           // mean payload entropy (from flow.payload_entropies or 0.0)
    pub cmd_entropy: f64,       // Shannon entropy of domain / flow_key (C2 command pattern)
}
...
pub struct BehavioralEngine {
    ...
    last_groom_time: f64,
    tx: mpsc::Sender<Arc<serde_json::Value>>,
    _rt: Runtime,
}

impl BehavioralEngine {
    fn new(tx: mpsc::Sender<Arc<serde_json::Value>>, rt: Runtime) -> Self {
    ...
    fn dispatch_alert_to_gateway(
        ...
        let payload = serde_json::json!({
            ...
            // c2_math 8D vector fields — computed by BehavioralEngine, stored here
            // so transmission/parquet.rs can populate the full vector without re-analysis.
            "outbound_ratio":    stats.outbound_ratio,
            "packet_size_mean":  stats.packet_size_mean,
            "packet_size_std":   stats.packet_size_std,
            "interval_mean":     stats.interval_mean,
            "cv":                stats.cv,
            "entropy":           stats.entropy,
            "cmd_entropy":       stats.cmd_entropy,
        });
        ...
        let _ = self.tx.try_send(Arc::new(payload));
    }
    ...
    pub fn evaluate_flow(&mut self, flow: IncomingTelemetry) -> Option<OutgoingAlert> {
        ...
        if n_intervals < 8 {
            if !flags.is_empty() {
                // Fast-path alert (DGA/FastFlux before volumetric analysis).
                // Compute minimal flow stats for vector fields; packet-level stats unavailable yet.
                let (early_mean_size, early_std_size) = MathEngine::calculate_mean_std(&flow.packet_sizes);
                let (early_mean_int, early_std_int) = MathEngine::calculate_mean_std(&flow.intervals);
                let early_cv = if early_mean_int > 0.0 { early_std_int / early_mean_int } else { 0.0 };
                let early_entropy = flow.payload_entropies.as_ref()
                    .map(|e| if e.is_empty() { 0.0 } else { e.iter().sum::<f64>() / e.len() as f64 })
                    .unwrap_or(0.0);
                let early_cmd_entropy = ThreatHeuristics::shannon_entropy(
                    flow.domain.as_deref().unwrap_or(&flow.key)
                );
                let early_outbound_ratio = flow.asymmetry_ratio
                    .map(|a| a / (1.0 + a))
                    .unwrap_or(0.75);

                let early_stats = FlowStats {
                    outbound_ratio: early_outbound_ratio,
                    packet_size_mean: early_mean_size,
                    packet_size_std: early_std_size,
                    interval_mean: early_mean_int,
                    cv: early_cv,
                    entropy: early_entropy,
                    cmd_entropy: early_cmd_entropy,
                };
                ...
                self.dispatch_alert_to_gateway(&alert, &flow.key, &flow.dst_ips, &flow.domain, &early_stats);
                return Some(alert);
            }
            return None;
        }
        ...
        // Assemble all c2_math vector features — computed above during volumetric analysis.
        let mean_payload_entropy = flow.payload_entropies.as_ref()
            .map(|e| if e.is_empty() { 0.0 } else { e.iter().sum::<f64>() / e.len() as f64 })
            .unwrap_or(0.0);
        let cmd_entropy = ThreatHeuristics::shannon_entropy(
            flow.domain.as_deref().unwrap_or(&flow.key)
        );
        let outbound_ratio = flow.asymmetry_ratio
            .map(|a| a / (1.0 + a))
            .unwrap_or(0.75);
        ...
        let stats = FlowStats {
            ...
            interval_mean: mean_int,
            cv: cv_interval,
            entropy: mean_payload_entropy,
            cmd_entropy,
        };
        ...
        self.dispatch_alert_to_gateway(&alert, &flow.key, &flow.dst_ips, &flow.domain, &stats);
        Some(alert)
    }
}
...
fn setup_custom_panic_hook() {
    ...
    // set_var is unsafe in edition 2024 (env mutation must not race other threads);
    // this runs during single-threaded FFI init before any worker starts.
    ...
    let _ = std::panic::catch_unwind(|| {
        ...
        if let Ok(json_str) = c_str.to_str() {
            if let Ok(val) = serde_json::from_str::<serde_json::Value>(json_str) {
                ...
                let _ = engine.tx.try_send(Arc::new(val));
            }
        }
    });
}
