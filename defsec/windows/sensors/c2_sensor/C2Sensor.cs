...
public class RealTimeC2Sensor {
    ...
    public static void TransmitAlertToGateway(string jsonPayload) {
        if (_mlEnginePtr != IntPtr.Zero && !string.IsNullOrEmpty(jsonPayload)) {
            try {
                ...
                SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "FFI TX ERROR: " + ex.Message });
            }
        }
    }
    ...
    private static void OnRustLog(IntPtr messagePtr) {
        if (messagePtr != IntPtr.Zero) {
            ...
            SafeEnqueue(new C2Event { Provider = "DiagLog", Message = msg });
        }
    }
    ...
    // Bounded enqueue: high-volume ETW providers (esp. NDIS-PacketCapture = every packet)
    // can fill EventQueue faster than the PowerShell consumer drains it. Without a cap the
    // queue grows unbounded -> pwsh RAM balloons to many GB and the UI starves. Shed load by
    // dropping new events once the backlog is saturated (drop-newest is cheap and lock-free);
    // DroppedEvents is exposed so the launcher can surface a saturation warning.
    private const int MaxQueueDepth = 50000;
    public static long DroppedEvents = 0;
    public static void SafeEnqueue(C2Event e) {
        if (EventQueue.Count >= MaxQueueDepth) {
            DroppedEvents++;   // diagnostic only; SafeEnqueue runs on the single ETW dispatch thread
            return;
        }
    ...
    // PID -> process name (from Kernel-Process Start). TCPIP/DNS events carry a PID but no resolved
    // ProcessName, so network telemetry (and thus UEBA baselines) logged "Unknown". Resolve from this.
    private static ConcurrentDictionary<int, string> ProcessNames = new ConcurrentDictionary<int, string>();
    ...
    public static string EvaluateBatch(string jsonPayload) {
        ...
        try {
            ...
            SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "FFI MARSHAL ERROR: " + ex.Message });
            return "{}";
        } finally {
        ...
        string[] suspiciousPaths)
    {
        ...
        try {
            ...
            if (_mlEnginePtr != IntPtr.Zero) {
                SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "[ML ENGINE] Native DLL successfully mapped at memory address: 0x" });
            } else {
                SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "[ML ENGINE ERROR] init_engine returned NULL." });
            }
        } catch (Exception ex) {
            SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "[ML ENGINE ERROR] FFI Import Failed: " + EscapeJson(ex.Message) });
        }
        ...
        if (maliciousIps != null && maliciousDomains != null) {
            ...
            SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "[THREAT INTEL] Binary Search arrays loaded: " + CompiledIps.Length + " IPs, " + CompiledDomains.Length + " Domains." });
        }
    ...
    // RFC1918 private ranges ONLY: 10/8, 172.16-31/12, 192.168/16. Bare "172." is wrong -- it
    // would treat public 172.x (e.g. Google 172.253.x) as internal and drop that egress (blindness).
    private static bool IsRfc1918(string ip) {
        ...
        if (ip.StartsWith("172.")) {
            ...
            int oct2;
            if (dot > 4 && int.TryParse(ip.Substring(4, dot - 4), out oct2)) return oct2 >= 16 && oct2 <= 31;
        }
        return false;
    }
                    ...
                    try {
                        ...
                        if (data.ProviderName.Contains("Kernel-Process") && data.EventName.Contains("Start")) {
                            ...
                            if (cmd.Contains("logman") && (cmd.Contains("stop") || cmd.Contains("delete")) && cmd.Contains("c2realtimesession")) {
                                SafeEnqueue(new C2Event { Provider = "TamperGuard", EventName = "ETW_STOP_ATTEMPT", Details = "A process attempted to blind the C2 ETW Session via Logman." });
                            }
                        }
                            ...
                            if (protectionObj != null) {
                                ...
                                if (protection == 0x40) {
                                    string proc = string.IsNullOrEmpty(data.ProcessName) ? data.ProcessID.ToString() : data.ProcessName;
                                        SafeEnqueue(new C2Event { Provider = "TamperGuard", EventName = "MEMORY_PATCH_DETECTED", Details = "Suspicious RWX permission change detected in process: " + proc });                              }
                            }
                        }
                        ...
                        if (data.ProviderName.Contains("Kernel-Process")) {
                            if (data.EventName.Contains("Start")) {
                                ...
                                // Cache PID -> process name so TCPIP/DNS network events (which lack ProcessName)
                                // can attribute the owning process in telemetry + UEBA instead of "Unknown".
                                string imageName = System.IO.Path.GetFileName(data.PayloadStringByName("ImageFileName") ?? "");
                                if (!string.IsNullOrEmpty(imageName)) {
                                    ProcessNames[data.ProcessID] = imageName;
                                }
                                ...
                                ProcessNames.TryRemove(data.ProcessID, out removedContext);
                            }
                            ...
                            if (data.EventName.Contains("Start")) {
                                ...
                                if (isWebParent || isDbParent) {
                                    ...
                                    if (isInterpreter || isSuspiciousPath) {
                                        ...
                                        SafeEnqueue(new C2Event { Provider = "AppGuard", EventName = eventType, Parent = parentContext, Child = childClean, Trigger = trigger, CommandLine = childCmdLine });
                                    }
                                }
                            }
                        }
                            ...
                            try {
                                ...
                                if (payloadLength > 5) {
                                    ...
                                    if (!string.IsNullOrEmpty(ja3Hash)) {
                                        ...
                                        SafeEnqueue(new C2Event { Provider = "NDIS", EventName = "TLS_JA3_FINGERPRINT", DestIp = ndisDestIp, Port = destPort.ToString(), JA3 = ja3Hash });
                                    }
                                }
                            } catch {
                            }
                        ...
                        // KNOWN GAP (verified 2026-07-09): the named-pipe rule below is effectively DORMANT.
                        // Microsoft-Windows-Kernel-File does NOT instrument NPFS - its Create/NameCreate events fire
                        // only for disk files, never named pipes (confirmed: 0 pipe events across all KF event types).
                        // No manifest ETW provider surfaces pipe creation; reliable detection needs a minifilter
                        // (e.g. Sysmon EID 17/18) or the unified .NET10/Rust XDR. Active enumeration is unsafe (can
                        // wedge the network stack), so it is intentionally not used. Pattern kept for if events arrive.
                            ...
                            if (fileName.Contains("\\device\\namedpipe\\") || fileName.Contains("\\pipe\\")) {
                                foreach (string pipePattern in MaliciousPipes) {
                                    if (fileName.Contains(pipePattern)) {
                                        ...
                                        SafeEnqueue(new C2Event { Provider = "P2P_Guard", EventName = "MALICIOUS_PIPE_CREATED", Image = pipeProc, CommandLine = fileName, SuspiciousFlags = "Known C2 Named Pipe" });
                                        break;
                                    }
                                }
                            }
                        ...
                        string image = data.ProcessName;
                        if (string.IsNullOrEmpty(image)) {
                            // TCPIP/DNS events carry only a PID. Resolve PID->name: Kernel-Process cache first (new
                            // processes), else query the live process table once and cache it. Most network traffic is
                            // from pre-existing long-running processes (browsers/svchost) that were never seen starting,
                            // so they aren't in the ProcessStart cache -> without this they logged as "Unknown".
                            string cachedName;
                            if (!ProcessNames.TryGetValue(data.ProcessID, out cachedName)) {
                                try { cachedName = System.Diagnostics.Process.GetProcessById(data.ProcessID).ProcessName; }
                                catch { cachedName = "Unknown"; }
                                ProcessNames[data.ProcessID] = cachedName;
                            }
                            image = cachedName;
                        }
                        ...
                        string cmdLine = "";
                        string cachedCmd;
                        if (ProcessCmdLines.TryGetValue(data.ProcessID, out cachedCmd)) {
                            cmdLine = cachedCmd;
                        }
                        ...
                        if (isNetworkEvent) {
                            bool isPrivateIp = IsRfc1918(destIp);
                        ...
                        if (isNetworkEvent && !string.IsNullOrEmpty(destIp)) {
                            ...
                            if (ipVal != 0 && Array.BinarySearch(CompiledIps, ipVal) >= 0) {
                                string ruleName;
                                TiContext.TryGetValue(destIp, out ruleName);
                        ...
                        if (string.IsNullOrEmpty(threatIntelTag) && data.ProviderName.Contains("DNS") && !string.IsNullOrEmpty(query)) {
                            ...
                            while (idx < cleanQuery.Length) {
                                ...
                                if (domHash != 0 && Array.BinarySearch(CompiledDomains, domHash) >= 0) {
                                    ...
                                    string ruleName;
                                    TiContext.TryGetValue(matchedSub, out ruleName);
                        ...
                        try {
                            SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "[ETW HOTPATH ERROR] " + EscapeJson(ex.Message) });
                        } catch { /* prevent recursive failure if enqueue itself fails */ }
                    }
                };
                _session.Source.Process();
            } catch (Exception ex) {
                SafeEnqueue(new C2Event { Error = EscapeJson(ex.Message) });
            }
        });
    ...
    public static void StopSession() {
        ...
        if (_mlEnginePtr != IntPtr.Zero) {
            ...
            SafeEnqueue(new C2Event { Provider = "DiagLog", Message = "[ML ENGINE] Native Rust DLL safely unloaded and DB flushed." });
        }
    }
