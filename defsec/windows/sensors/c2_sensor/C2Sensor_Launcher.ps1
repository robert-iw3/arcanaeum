...
# Wire the -EnableDiagnostics switch to the global that Write-Diag gates on (line ~841).
...
'@
    ...
    $RunspaceCode = {
        ...
        if (-not $RunInBackground) {
            # Sensor runs ELEVATED; a plain Start-Process would launch the browser elevated and
            # collide with the user's medium-integrity browser session (profile singleton lock),
            # wedging it -- hard-fails under STIG UAC/integrity policy. explorer.exe runs at medium
            # integrity as the user, launching the browser DE-ELEVATED and decoupled.
...
# PowerShell-native artifact correlation (no Rust FFI dependency -- c2 isn't hitting the
...
function Write-ForensicLedgerEntry {
    ...
    try {
        $ReportDir = "C:\ProgramData\C2Sensor\Data\Reports"
        ...
        $LedgerPath = "C:\ProgramData\C2Sensor\Data\ArmedMode_ForensicLedger.jsonl"
        $ledgerEntry = [ordered]@{
    ...
    } catch { Write-EngineDiag -Message "[ACTIVE DEFENSE] Failed to append forensic ledger: $($_.Exception.Message)" -Level "WARN" }
}
...
function Invoke-ActiveDefense($ProcName, $DestIp, $Confidence, $Reason, [int]$PID_Id = 0) {
    ...
    if (-not $justified) {
        Write-EngineDiag -Message "[ACTIVE DEFENSE] $ProcName (PID $PID_Id) held pending corroboration -- single alert, Confidence=$Confidence, no prior actor activity in window." -Level "WARN"
    ...
    # PID-scoped process block -- stops this specific binary from any further egress, not just
    # the one destination IP (an attacker can pivot to a different C2 host). Outbound only: C2
    # beaconing is inherently an outbound phenomenon.
    $procBlockStatus = ""
    if ($onDiskArtifacts.ImagePath -and $onDiskArtifacts.ImagePath -ne "Unknown" -and (Test-Path $onDiskArtifacts.ImagePath -ErrorAction SilentlyContinue)) {
        try {
            ...
            $ruleName2 = "C2_Defend_PID_${PID_Id}_${tag}"
            New-NetFirewallRule -DisplayName $ruleName2 -Direction Outbound -Action Block -Program $onDiskArtifacts.ImagePath -ErrorAction Stop | Out-Null
            $procBlockStatus = " | Process Network-Blocked ($($onDiskArtifacts.ImagePath))"
            $global:TotalMitigations++
        } catch { Write-EngineDiag -Message "[ACTIVE DEFENSE ERROR] Failed to create process-scoped firewall rule: $($_.Exception.Message)" -Level "WARN" }
    }
...
function Submit-SensorAlert {
    param(
        ...
        [switch]$IsSuppressed, [int]$PID_Id = 0
    )
...
$DotNetTarget = if ($PSVersionTable.PSVersion.Major -ge 7) { "netstandard2.0" } else { "net462" }
...
try {
    ...
    while ($true) {
            ...
            if ($props.SuspiciousFlags.Count -gt 0) {
                ...
                Submit-SensorAlert -Type "Static_Detection" -Destination $safeDestStatic -Image $procName -Flags ($props.SuspiciousFlags -join '; ') -Confidence 90 -AttckMapping ($props.ATTCKMappings -join '; ') -RawJson $enrichedJson -CommandLine $staticCmd -PID_Id $staticPidInt
            }
        }
        ...
        if ([RealTimeC2Sensor]::EventQueue.IsEmpty) { Start-Sleep -Milliseconds 200 }
    }
