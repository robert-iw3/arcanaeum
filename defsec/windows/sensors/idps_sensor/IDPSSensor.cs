...
public class RealTimeIDPSSensor {
    ...
    // Forward an enriched IDPS alert to the Rust transmission worker
    // (SQLite WAL -> Parquet -> gateway).
    public static void TransmitAlertToGateway(string jsonPayload) {
        if (_mlEnginePtr != IntPtr.Zero && !string.IsNullOrEmpty(jsonPayload)) {
            try {
                ...
                SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = "FFI TX ERROR: " + ex.Message });
            }
        }
    }
    ...
    // Bounded enqueue: high-volume ETW providers (esp. NDIS-PacketCapture = every packet) can fill
    // EventQueue faster than the PowerShell consumer drains it. Without a cap the queue grows
    // unbounded -> pwsh RAM balloons to many GB and the UI starves. Shed load by dropping new events
    // once saturated (drop-newest is cheap and lock-free); DroppedEvents exposed for a saturation warning.
    private const int MaxQueueDepth = 50000;
    public static long DroppedEvents = 0;
    public static void SafeEnqueue(IDPSEvent e) {
    ...
    public static string EvaluateBatch(string jsonPayload) {
        ...
        try {
            ...
            SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = $"FFI MARSHAL ERROR: {ex.Message}" });
            return "{}";
        } finally {
        ...
        string[] suspiciousPaths)
    {
        ...
        try {
            ...
            if (_mlEnginePtr != IntPtr.Zero) {
                SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE] Native DLL successfully mapped at memory address: 0x" });
            } else {
                SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE ERROR] init_engine returned NULL." });
            }
        } catch (Exception ex) {
            SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE ERROR] FFI Import Failed: " + EscapeJson(ex.Message) });
        }
        ...
        if (maliciousIps != null && maliciousDomains != null) {
            ...
            SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = $"[THREAT INTEL] Binary Search arrays loaded: {CompiledIps.Length} IPs, {CompiledDomains.Length} Domains." });
        }
                    ...
                    try {
                        ...
                        if (data.ProviderName.Contains("Kernel-Process") && data.EventName.Contains("Start")) {
                            ...
                            if (cmd.Contains("logman") && (cmd.Contains("stop") || cmd.Contains("delete")) && cmd.Contains("idpsrealtimesession")) {
                                SafeEnqueue(new IDPSEvent { Provider = "TamperGuard", EventName = "ETW_STOP_ATTEMPT", Details = "A process attempted to blind the IDPS ETW Session via Logman." });
                            }
                        }
                            ...
                            if (protectionObj != null) {
                                ...
                                if (protection == 0x40) {
                                    string proc = string.IsNullOrEmpty(data.ProcessName) ? data.ProcessID.ToString() : data.ProcessName;
                                        SafeEnqueue(new IDPSEvent { Provider = "TamperGuard", EventName = "MEMORY_PATCH_DETECTED", Details = "Suspicious RWX permission change detected in process: " + proc });                              }
                            }
                        }
                            ...
                            if (data.EventName.Contains("Start")) {
                                ...
                                if (isWebParent || isDbParent) {
                                    ...
                                    if (isInterpreter || isSuspiciousPath) {
                                        ...
                                        SafeEnqueue(new IDPSEvent { Provider = "AppGuard", EventName = eventType, Parent = parentContext, Child = childClean, Trigger = trigger, CommandLine = childCmdLine });
                                    }
                                }
                            }
                        }
                            ...
                            try {
                                ...
                                if (payloadLength > 5) {
                                    ...
                                    if (!string.IsNullOrEmpty(ja3Hash)) {
                                        ...
                                        SafeEnqueue(new IDPSEvent { Provider = "NDIS", EventName = "TLS_JA3_FINGERPRINT", DestIp = ndisDestIp, Port = destPort.ToString(), JA3 = ja3Hash });
                                    }
                                }
                            } catch {
                            }
                            ...
                            if (fileName.Contains("\\device\\namedpipe\\") || fileName.Contains("\\pipe\\")) {
                                foreach (string pipePattern in MaliciousPipes) {
                                    if (fileName.Contains(pipePattern)) {
                                        ...
                                        SafeEnqueue(new IDPSEvent { Provider = "P2P_Guard", EventName = "MALICIOUS_PIPE_CREATED", Image = pipeProc, CommandLine = fileName, SuspiciousFlags = "Known C2 Named Pipe" });
                                        break;
                                    }
                                }
                            }
                        ...
                        if (isNetworkEvent) {
                            ...
                            bool isDestPrivate = IsRfc1918(destIp);
                            bool isSrcPrivate = IsRfc1918(srcIp);
                        ...
                        try {
                            SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ETW HOTPATH ERROR] " + EscapeJson(ex.Message) });
                        } catch { /* prevent recursive failure if enqueue itself fails */ }
                    }
                };
                _session.Source.Process();
            } catch (Exception ex) {
                SafeEnqueue(new IDPSEvent { Error = EscapeJson(ex.Message) });
            }
        });
    ...
    public static void StopSession() {
        ...
        if (_mlEnginePtr != IntPtr.Zero) {
            ...
            SafeEnqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE] Native Rust DLL safely unloaded and DB flushed." });
        }
    }
