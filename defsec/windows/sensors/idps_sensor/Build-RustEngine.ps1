<#
.SYNOPSIS
    IDPS Sensor -- Rust Workspace Compiler Pipeline

.DESCRIPTION
    Compiles the IDPSSensor Rust workspace into a native C-compatible DLL
    (idpssensor_ml.dll) for direct FFI integration with the C# ETW orchestrator.

    The workspace contains two crates:
      * ml_engine    -- The behavioral ML engine (cdylib -> idpssensor_ml.dll)
      * transmission -- Async telemetry pusher to the sensor middleware gateway
                        (SQLite WAL -> ZSTD Parquet -> HMAC-stamped gateway upload)
...
Write-Host "$cCyan$cBold║   IDPS SENSOR -- RUST WORKSPACE COMPILER PIPELINE            ║$cReset"
Write-Host "$cCyan$cBold╚══════════════════════════════════════════════════════════════╝$cReset"
Write-Host ""
...
if (-not (Test-Path $wsCargoToml)) {
    ...
    Write-Host "    The build script must be run from the IDPSSensor workspace root."
    Exit 1
}
...
if (-not (Test-Path $CompiledDll)) {
    ...
    Write-Host "    Check that ml_engine/Cargo.toml [lib] name = 'idpssensor_ml' and crate-type = ['cdylib']"
    Exit 1
}
...
Write-Host ""
Write-Host "$cCyan$cBold╔══════════════════════════════════════════════════════════════╗$cReset"
Write-Host "$cCyan║$cReset $cGreen SUCCESS: IDPS Sensor ML Engine Compiled$cReset"
