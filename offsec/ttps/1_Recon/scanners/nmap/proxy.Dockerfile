...
RUN \
    ...
    apk add --no-cache \
        ...
        gcc gcompat git \
        iptables iputils \
        libcrypto3 libffi-dev libgcc linux-headers libpcap libssh2 libssl3 libstdc++ libxml2 lua5.4-libs \
        musl-dev \
        openssl \
        privoxy proxychains-ng psutils python3-dev py3-pip \
        shadow \
        tini tor tzdata \
        zlib \
    ; \
